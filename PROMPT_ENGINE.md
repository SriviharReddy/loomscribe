...
**System-slot parameters** (POV, pacing, prose style, explicit, internal monologue) live in Slot 1. Changing them mid-conversation invalidates the KV cache on the next send. The **amber dot** indicator in the Conversation Settings panel flags which parameters have changed since the last send.
...
| `content` | explicit, internal_monologue | Toggled by `explicit` and `internal_mono` params |
...
```json
{
  ...
  "system_body": "300–500 words of scenario-specific content. The genre lens, the erotic or dramatic charge, the range of sub-scenarios, any scenario-specific behaviours the model must maintain.",
  ...
  "blocks": [
    ...
    { "id": "explicit",           "enabled": true,  "order": 40 },
...
| `explicit` | toggle | system | Enables or disables the explicit block |
...
- The specific erotic or dramatic charge — what creates tension
