...
The baseline `blocks` list above covers only the **core structural blocks** that every preset must explicitly own. The remaining blocks are organised into **compiler-resolved groups** — the compiler selects exactly one block per group at runtime based on the active parameter value. You do **not** list them in the preset's `blocks` array unless you need to hard-override a group for this specific preset.
